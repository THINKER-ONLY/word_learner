...
class WordManager:
    ...
    def find_word(self, value_to_find, key=None):
        """
        灵活查找单词。
        - 如果提供了 key (如 'english' 或 'chinese')，则进行精确的键值匹配。
        - 如果未提供 key，则默认在 'english' 和 'chinese' 两个字段中查找。
        ...
        if key:
            ...
            # 未提供key，在 'english' 和 'chinese' 中查找
            for word in self.words:
                if word.get('english') == value_to_find or word.get('chinese') == value_to_find:
                    return word
        return None
    ...
    def add_word(self, en, cn, mean=""):
        ...
        :param en: 英文单词。
        :param cn: 中文释义。
        :param mean: 其他含义或备注 (根据您的设计)。
        :return: 如果添加成功返回 True，如果单词已存在则返回 False。
        """
        if self.find_word(en, key='english'):
            print(f"添加失败: 单词 '{en}' 已存在。")
            return False
    ...
    def edit_word(self, original_word_en, updates):
        """
        编辑一个现有的单词。
        此版本遵循"缓冲区"模式，只修改内存中的数据并设置脏位。

        :param original_word_en: 要编辑的单词的原始英文 (例如: 'apple')。
        :param updates: 一个包含更新信息的字典。
                         例如: {'chinese': '苹果公司'} 或 {'english': 'apples', 'meaning': 'n. pl.'}
        ...
        word_to_edit = self.find_word(original_word_en, key='english')

        if not word_to_edit:
            print(f"编辑失败: 在库中未找到单词 '{original_word_en}'。")
            return False

        # 如果要修改英文单词本身，需要确保新单词不会与库中其他单词重复。
        new_english = updates.get('english')
        if new_english and new_english != original_word_en:
            if self.find_word(new_english, key='english'):
                print(f"编辑失败: 新单词 '{new_english}' 已经存在于库中。")
                return False
    ...
    def delete_word(self, english_to_delete):
        """
        根据英文单词从库中删除一个单词。
        :param english_to_delete: 要删除的单词的英文。
        :return: 如果删除成功返回 True，否则返回 False。
        """
        original_count = len(self.words)
        self.words = [word for word in self.words if word.get('english') != english_to_delete]
        
        if len(self.words) < original_count:
            self.is_dirty = True
            return True
        else:
            print(f"删除失败: 未找到单词 '{english_to_delete}'。")
            return False